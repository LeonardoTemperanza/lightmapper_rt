...
void main()
{
    ...
    base_color_ = texture(sampler2D(_res_textures_[nonuniformEXT(data_._res_.base_color_map)], _res_samplers_[nonuniformEXT(data_._res_.base_color_map_sampler)]), uv_);
    if((base_color_.w < 0.1))
    {
        discard;
    }

    normal_map_sample_ = texture(sampler2D(_res_textures_[nonuniformEXT(data_._res_.normal_map)], _res_samplers_[nonuniformEXT(data_._res_.normal_map_sampler)]), uv_);
    world_normal_ = normalize(normal_vert_.xyz);
    packed_normal_ = vec4(((world_normal_ * 0.5) + 0.5), 1.0);
    metallic_roughness_sample_ = texture(sampler2D(_res_textures_[nonuniformEXT(data_._res_.metallic_roughness_map)], _res_samplers_[nonuniformEXT(data_._res_.metallic_roughness_map_sampler)]), uv_);
    albedo_ = base_color_;
